...
class DiT(nn.Module):
    def __init__(
        ...
        super().__init__()
        proj_dim = hidden_size if proj_dim is None else proj_dim
        ...
        self.rin_merge = nn.Sequential(
            nn.LayerNorm(hidden_size*2, bias=False),
            nn.Linear(hidden_size * 2, hidden_size, bias=False),
        )

        self.proj_out = nn.Linear(hidden_size, proj_dim, bias=False)
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, **kwargs) -> Dict[str, torch.Tensor]:
        class_drop_prob = kwargs.get('class_drop_prob', 0.0)
        force_routing = kwargs.get('force_routing', False)
        rin_features_in = kwargs.get('rin_features_in', None)
        overwrite_selection_ratio = kwargs.get('overwrite_selection_ratio', None)
        feat_layer = kwargs.get('feat_layer', None)
        should_project = kwargs.get('should_project', True)

         
        x = self.x_embedder(x)
        x = self.rin_merge(torch.cat([x, rin_features_in], dim=-1))
        x = x + self.pos_embed
        t = self.t_embedder(t)
        c = t 
        ...
        for idx, block in enumerate(self.blocks):
            if use_routing and idx == self.routes[route_count]['start_layer_idx']:
                x_D_last = x.clone()
                mask_info = self.router.get_mask(x, selection_rate=self.routes[route_count]['selection_ratio'] if overwrite_selection_ratio is None else overwrite_selection_ratio)
            ...
            if idx == feat_layer:
                rin_features_out = self.proj_out(x) if should_project else x
        ...
        return self.unpatchify(x), rin_features_out
...
def DiT_XXL_2(**kwargs):
    return DiT(depth=36, hidden_size=1280, patch_size=2, num_heads=20, **kwargs)
